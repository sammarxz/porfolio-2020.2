function SplitText(i, vars) {
  ...
  if (vars && typeof vars === 'object' && vars !== null) {
    ...
    if (vars.type && typeof vars.type === 'string') {
      ...
      const possible = ['chars', 'words', 'lines']
      const use = []
  ...
  for (var i = 0; i < this.HTMLobjects.length; i++) {
    const current = this.HTMLobjects[i]
    ...
    if (this.vars.type.indexOf('lines') !== -1) {
      ...
      for (var j = 0; j < splitPoints.length; j++) {
        const lineStart = j === 0 ? 0 : splitPoints[j - 1] + 1
        const lineEnd =
          j === splitPoints.length - 1 ? text.length : splitPoints[j]

        const div = document.createElement('div')

        div.classList.add('line', 'd--block', 'p--relative')
        if (
        ...
        div.innerHTML = `<span class="line__content d--block">${text.substring(
          lineStart,
          lineEnd
        )}</span>`
    ...
    if (this.vars.type.indexOf('words') !== -1) {
      function splitWords(parent, st) {
        const startTag = "<div style='display:inline-block;'>"
        const endTag = '</div>'
        parent.innerHTML =
          startTag +
    ...
    if (this.vars.type.indexOf('chars') !== -1) {
      function splitChars(parent, st) {
        const startTag = "<div style='display:inline-block;'>"
        const endTag = '</div>'
        const specials = parent.innerHTML.match(/(&\w+;)/g)
        parent.innerHTML =
          startTag +
          parent.innerHTML
            .replace(/&\w+;/g, 'ህ')
        ...
        for (let j = 0; j < nodes.length; j++) {
          if (
            ...
            let newClass = st.vars.charsClass.replaceAll('++', j + 1)
            if (j !== nodes.length - 1) {
    ...
    if (this.vars.position === 'absolute' || this.vars.position === 'fixed') {
      for (var j = currentLists.chars.length - 1; j >= 0; j--) {
        currentLists.chars[
          j
        ].style.width = `${currentLists.chars[j].offsetWidth}px`
        currentLists.chars[
          j
        ].style.height = `${currentLists.chars[j].offsetHeight}px`
        currentLists.chars[
          j
        ].style.left = `${currentLists.chars[j].toBe.left}px`
      ...
      for (var j = currentLists.words.length - 1; j >= 0; j--) {
        currentLists.words[
          j
        ].style.width = `${currentLists.words[j].offsetWidth}px`
        currentLists.words[
          j
        ].style.height = `${currentLists.words[j].offsetHeight}px`
        currentLists.words[
          j
        ].style.left = `${currentLists.words[j].toBe.left}px`
      ...
      for (var j = currentLists.lines.length - 1; j >= 0; j--) {
        currentLists.lines[
          j
        ].style.width = `${currentLists.lines[j].offsetWidth}px`
        currentLists.lines[
          j
        ].style.height = `${currentLists.lines[j].offsetHeight}px`
        currentLists.lines[
          j
        ].style.left = `${currentLists.lines[j].toBe.left}px`
